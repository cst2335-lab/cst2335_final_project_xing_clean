...
class _$CustomerDao extends CustomerDao {
  ...
  Future<List<Customer>> findAllCustomers() async {
    return _queryAdapter.queryList(
        'SELECT * FROM Customer ORDER BY lastName ASC, firstName ASC',
  ...
  @override
  Future<int?> countDuplicateCustomers(
    String firstName,
    String lastName,
    String address,
  ) async {
    return _queryAdapter.query(
        'SELECT COUNT(*) FROM Customer WHERE firstName = ?1 AND lastName = ?2 AND address = ?3',
        mapper: (Map<String, Object?> row) => row.values.first as int,
        arguments: [firstName, lastName, address]);
  }
